...
export const featureOptions: { [index: string]: AccessFeatureOption[] } = {
  ...
  "Hub": [
    ...
    { default: true, description: "Add a lock accessory for the side door on UA Hub Gate devices.", modelKey: ["UA Hub Gate"], name: "SideDoor" },
    { default: false, defaultValue: ACCESS_DEVICE_UNLOCK_INTERVAL, description: "Delay, in minutes, before locking the side door lock relay, once it's been unlocked by HomeKit. If set to 0, it will remain unlocked indefinitely. By default, the side door lock relay will lock five seconds after unlocking.", modelKey: ["UA Hub Gate"], name: "SideDoor.LockDelayInterval" },
    { default: false, description: "Add a switch accessory to control the side door lock. This can be useful in automation scenarios where you want to work around HomeKit's security restrictions for controlling locks.", modelKey: ["UA Hub Gate"], name: "SideDoor.Lock.Trigger" }
  ],
  ...
  "Log": [
    ...
    { default: true, description: "Log side door lock events in Homebridge.", modelKey: ["UA Hub Gate"], name: "SideDoorLock" }
  ]
};
